...
LOGITS_MEAN = 0.0
LOGITS_STD  = 0.8   # based on your previous stats
...
class MNISTDiffusion(nn.Module):
    ...
    # ---------- y_0 (logits) from x_0 ----------
    # @torch.no_grad()
    # def get_y_0(self, noisy_image, x_0):
    #     with torch.no_grad():
    ...
    #     # iterative EM-like refinement between x and y
    #     y_0 = torch.zeros(n_samples, self.num_classes, device=device)
    #     for _ in range(self.iteration_steps):
    #         x_0 = self.get_x_0(x_cond, y_0).to(device)
    #         y_0 = self.get_y_0(x_cond, x_0).to(device)
    ...
    def _reverse_diffusion_with_clip(self, x_t, t, noise, x_cond=None, y_cond=None, is_x=True):
        if is_x:
            ...
            # No softmax here; caller can interpret as logits if needed
            if self.is_ddim:
                std = 0.0
