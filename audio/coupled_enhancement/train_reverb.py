...
def load_pretrained_diffusion_model(
    ...
    # --- copy only matching keys & shapes ---
    pretrained_filtered = {}
    new_parameters = []
        ...
        if name in pretrained_state_dict:
            ...
            if param.shape == pretrained_param.shape:
                ...
                print(f"[shape mismatch] {name}: "
                      f"model={tuple(param.shape)}, ckpt={tuple(pretrained_param.shape)} "
                      f"-> keep random init")
                new_parameters.append(name)
        else:
    ...
    model_state.update(pretrained_filtered)
    ...
    if freeze_pretrained:
        ...
        for name, param in model.named_parameters():
            if name in pretrained_filtered:
                param.requires_grad = False
                frozen_count += 1
        print(f"Frozen {frozen_count} parameters that came from the pretrained checkpoint.")
    ...
    # Debug: show trainable vs total params
...
if __name__ == '__main__':
     ...
     for parser_ in (base_parser, parser):
          parser_.add_argument("--backbone", type=str, choices=BackboneRegistry.get_all_names(), default="ncsnpp_48k_logits_conditioned")
          ...
          parser_.add_argument("--logits_pretrain_ckpt", type=str, default="/mlspeech/data/gilad/logs/ASR_diffusion_ears/reverb/with_logits/logits_model_pretrain_width_768_lr_1e-4_score_pred_residual_model_complex_audio_and_prev_logits_baseline_weight-best.ckpt",
                    help="Where to save/load the pretrained logits_model.")
          parser_.add_argument("--log_dir", type=str, default="/mlspeech/data/gilad/logs/ASR_diffusion_ears/reverb/with_logits", help="Directory to save logs.")
          ...
          parser_.add_argument("--logits_weight", type=float, default=1.0,
                              help="Weight for logits loss (only for logits mode)")
          parser_.add_argument("--whisper_lang", type=str, default="en",
                              help="Language for Whisper decoding")
          
          parser_.add_argument("--freeze_pretrained", action='store_true', 
                    help="Freeze pretrained parameters during fine-tuning.")
          parser_.add_argument("--warmup_steps", type=int, default=0,
                              help="Number of warmup steps before unfreezing pretrained parameters.")
          parser_.add_argument("--no_ema_validation", action='store_true',
                              help="Disable EMA during validation for direct comparison.")
     ...
     model = WhisperGuidedScoreModel(
          ...
          logits_pretrain_ckpt=args.logits_pretrain_ckpt,
          **{
     ...
     ddp = DDPStrategy(find_unused_parameters=False, timeout=timedelta(hours=72))
     trainer = pl.Trainer(
     **vars(arg_groups["Trainer"]),
     ...
     if args.mode == "train":
          ...
          if args.ckpt is None:
               raise ValueError("Checkpoint path must be provided when mode=valid")
          trainer.validate(model=model, ckpt_path=args.ckpt)

     elif args.mode == "test":
          if args.ckpt is None:
               raise ValueError("Checkpoint path must be provided when mode=test")
          trainer.test(model=model, ckpt_path=args.ckpt)
