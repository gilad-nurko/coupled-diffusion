    ...
    def update_fn(self, x, y, t, is_logits=False, logits=None, logits_cond=None, audio_embedding=None, *args):
        ...
        if is_logits:
            for _ in range(n_steps):
                ...
                grad = self.score_fn(logits, logits_cond, t, is_logits=is_logits, noisy_audio_embedding=audio_embedding, *args)
                noise = torch.randn_like(logits)
                step_size = (target_snr * std) ** 2 * 2
                logits_mean = logits + step_size[:, None, None] * grad
                logits = logits_mean + noise * torch.sqrt(step_size * 2)[:, None, None]
                return logits, logits_mean
        else:
            for _ in range(n_steps):
                std = self.sde.marginal_prob(x, y, t, is_logits=False, *args)[1]
                grad = self.score_fn(x, y, t, is_logits=is_logits, logits_cond=logits ,*args)
                noise = torch.randn_like(x)
                step_size = (target_snr * std) ** 2 * 2
                x_mean = x + step_size[:, None, None, None] * grad
                x = x_mean + noise * torch.sqrt(step_size * 2)[:, None, None, None]
