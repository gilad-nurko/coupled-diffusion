...
from sgmse.backbones.logits_model import LogitScoreNet
...
class WhisperGuidedScoreModel(pl.LightningModule):
    ...
    def __init__(
        ...
        num_iterations=5, logits_diffusion_steps=10, **kwargs
    ):
        """
        ...
        self.logits_model = LogitScoreNet(
            num_classes=len(self.allowed_words),
            timesteps=self.sde.N
        )
    ...
    def on_fit_start(self):
        ...
        def extract_logits(x_t, lengths, multilingual_tokens, keyword_idx, batch_idx):
            # x_t: (B, 1, F, T) (likely CPU). Convert to audio on CPU, then move to Whisper device.
            x_t_audio_list_padded = []
            for i in range(x_t.shape[0]):
                spec_single = x_t[i].squeeze(0)  # (F, T)
                audio_single = self.to_audio(spec_single, lengths[i].item()).to(dtype=torch.float32)
                x_t_audio_list_padded.append(self._pad_or_trim(audio_single))

            # (B, T) on Whisper device
            x_t_audio_padded = torch.stack(x_t_audio_list_padded, dim=0).to(whisper_dev)
            x_t_mel = self._log_mel_spectrogram(x_t_audio_padded)  # stays on whisper_dev

            feats  = self.whisper.encoder(x_t_mel)
            tokens = multilingual_tokens.to(whisper_dev)
            logits = self.whisper.decoder(tokens, feats)  # (B, L, V)

            # indexing tensors must be on the same device as logits
            bi = batch_idx.to(whisper_dev)
            ki = keyword_idx.to(whisper_dev)

            kw = logits[bi, ki][:, self.allowed_toks.to(whisper_dev)]
            kw_probs = torch.softmax(kw, dim=-1)
            return kw_probs
        ...
        for epoch in range(epochs):
            ...
            for batch in self.data_module.train_dataloader():
                specs, multilingual_tokens, labels, lengths = (
                    batch["specs"], batch["multilingual_tokens"], batch["labels"], batch["lengths"]
                )
                y = specs[:, 0:1]        # (B, 1, F, T)
                x = specs[:, 1:2]        # (B, 1, F, T)
                ...
                special = torch.tensor(list(special_ids), device=labels.device)
                mask = ~torch.isin(labels, special)          # True where token NOT special  (B,T)
                keyword_idx = mask.float().argmax(dim=1)     # position of first True per row
                batch_idx   = torch.arange(labels.size(0), device=labels.device)
                keyword_tok = labels[batch_idx, keyword_idx]

                true_id = (keyword_tok.unsqueeze(1) == self.allowed_toks.to(labels.device)).nonzero(as_tuple=False)[:, 1]
                one_hot = F.one_hot(true_id, num_classes=len(self.allowed_words)).float().to(model_dev) 
                noise_logits = torch.randn_like(one_hot)

                kw_probs = extract_logits(y, lengths, multilingual_tokens, keyword_idx, batch_idx).to(model_dev)

                one_hot_4d  = one_hot[:, :, None, None]
                kw_probs_4d = kw_probs[:, :, None, None]
                mean_l, std_l = self.sde.marginal_prob(one_hot_4d, kw_probs_4d, t)  
                mean_l = mean_l.squeeze(-1).squeeze(-1)  # (B, C)
                x_t_l = mean_l + std_l[:, None] * noise_logits

                logits_optimizer.zero_grad() 
                predicted_score = self.logits_model(x_t_l, kw_probs, t)

                sigma_l = self.sde._std(t)[:, None].to(model_dev) 
                loss = 0.5 * torch.mean(torch.sum((predicted_score * sigma_l + noise_logits).pow(2), dim=-1))
    ...
    def _loss(self, forward_out_audio, forward_out_logits, t, x_t_audio, z_audio, mean_audio, x_audio, x_t_logits, z_logits, mean_logits, x_logits):
        """
        Different loss functions for training the score model
        """
        sigma_a = self.sde._std(t)[:, None, None, None]
        sigma_l = self.sde._std(t)[:, None]
        ...
        if self.loss_type == "score_matching":
            ...
            loss_logits = 0.5 * torch.mean(torch.sum(losses_l, dim=-1)) 
        else:
    ...
    def _step(self, batch, batch_idx):
        specs, multilingual_tokens, labels, lengths = batch["specs"], batch["multilingual_tokens"], batch["labels"], batch["lengths"]
        y = specs[:, 0:1]        # (B, 1, F, T)
        x = specs[:, 1:2]        # (B, 1, F, T)
        ...
        special = torch.tensor(list(special_ids), device=labels.device)          # (S,)
        mask    = ~torch.isin(labels, special)        # True where token NOT special  (B,T)
        keyword_idx = mask.float().argmax(dim=1)      # position of first True per row
        batch_idx   = torch.arange(labels.size(0), device=labels.device)         # 0..B-1
        keyword_tok = labels[batch_idx, keyword_idx]                             # gather
        true_id = (keyword_tok.unsqueeze(1) == self.allowed_toks.to(labels.device)).nonzero(as_tuple=False)[:, 1]
        one_hot = F.one_hot(true_id, num_classes=len(self.allowed_words)).float()
        noise_logits = torch.randn_like(one_hot)

        def extract_logits(x_t, lengths, device, multilingual_tokens, keyword_idx, batch_idx):
            # prepare x_t for whisper - convert spectrogram to audio
            x_t_audio_list_padded = []
            for i in range(x_t.shape[0]):  # Process each batch item
                # Remove batch and channel dimensions: (1, F, T) -> (F, T)
                spec_single = x_t[i].squeeze(0)
                # Convert to audio using the data module's inverse transform
                audio_single = self.to_audio(spec_single, lengths[i].item())
                audio_single = audio_single.to(dtype=torch.float32, device=device)
                audio_single_padded = self._pad_or_trim(audio_single)  # Pad or trim to a fixed length
                x_t_audio_list_padded.append(audio_single_padded)
            
            # Stack the audio tensors
            x_t_audio_padded = torch.stack(x_t_audio_list_padded, dim=0)  # (B, T_audio)
            # x_t_audio = x_t_audio.to(dtype=torch.float32, device=labels.device)
            # signal_padded = self._pad_or_trim(x_t_audio)                  
            x_t_mel = self._log_mel_spectrogram(x_t_audio_padded)
            feats  = self.whisper.encoder(x_t_mel)                     
            logits = self.whisper.decoder(multilingual_tokens, feats)  # shapes (B,T*,C)
            kw_probs = logits[batch_idx, keyword_idx][:, self.allowed_toks.to(logits.device)]
            kw_probs = torch.softmax(kw_probs, dim=-1)
            return kw_probs
        
        if self.sampling_mode == "parallel":
            kw_probs = extract_logits(x_t, lengths, x.device, multilingual_tokens, keyword_idx, batch_idx)
        elif self.sampling_mode == "full" or self.sampling_mode == "nested":
            kw_probs = extract_logits(y, lengths, x.device, multilingual_tokens, keyword_idx, batch_idx)
        
        one_hot_4d  = one_hot[:,  :, None, None]
        kw_probs_4d = kw_probs[:, :, None, None]
        mean_l, std_l = self.sde.marginal_prob(one_hot_4d, kw_probs_4d, t)  # the noisy version is the whisper output on the noisy version
        mean_l = mean_l.squeeze(-1).squeeze(-1) # (B, C)
        x_t_l = mean_l + std_l[:, None] * noise_logits

        half_t = 0.5 * self.sde.T                    # scalar
        mask    = (t < half_t).unsqueeze(-1)         # (B, 1)  True ↔ late steps
        logits_cond = torch.where(mask, x_t_l, kw_probs)

        if self.sampling_mode == "parallel":
            forward_out_audio = self(x_t, y, t, is_logits=False, logits_cond=logits_cond)
            forward_out_logits = self(x_t_l, kw_probs, t, is_logits=True)
        elif self.sampling_mode == "full":
            forward_out_audio = self(x_t, y, t, is_logits=False, logits_cond=kw_probs)
            forward_out_logits = self(x_t_l, kw_probs, t, is_logits=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.num_eval_files != 0:
            ...
            if self.num_eval_files < 0:                      
                # eval_total = len(self.data_module.valid_set.clean_files)
                eval_total = len(ds.clean_files)
            else:                                          
                # eval_total = min(self.num_eval_files, len(self.data_module.valid_set.clean_files))
            ...
            # clean_files = self.data_module.valid_set.clean_files[:eval_total]
            # noisy_files = self.data_module.valid_set.noisy_files[:eval_total]
    ...
    def forward(self, x_t, y, t, is_logits=False, logits_cond=None):
        ...
        if is_logits:
            ...
            # In [1] and [2], we use the old code:
                ...
                if self.backbone == "ncsnpp_with_logits":
    ...
    def _plot_and_save_debug_info(self, clean_mel, corrupted_mel, enhanced_mel, clean_transcript, corrupted_transcript, enhanced_transcript, ground_truth, index):
        ...
        TIME_FRAMES = 100  # Number of time frames to display
