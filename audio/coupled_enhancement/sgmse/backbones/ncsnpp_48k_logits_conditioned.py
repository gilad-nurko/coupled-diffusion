...
class NCSNpp_48k_Logits(nn.Module):
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        self.logits_mid_cond = True
        self.logits_proj_channels = 64 
        self.logits_dim = logits_dim
        self.bottleneck_ch = self.nf * ch_mult[-1]
        ...
        # Per-step encoder over |A| -> C_cond
        self.mid_logits_encoder = nn.Sequential(
            nn.LayerNorm(self.logits_dim),
            nn.Linear(self.logits_dim, self.logits_proj_channels),
            get_act(nonlinearity),
            nn.Linear(self.logits_proj_channels, self.logits_proj_channels),
        )
        ...
        self.mid_gate = nn.Sequential(
            ...
            get_act(nonlinearity),
            nn.Linear(64, 1),
            nn.Sigmoid()  # Output in [0, 1], can be replaced with Softplus or removed
        )
        nn.init.zeros_(self.mid_gate[-2].weight)
        nn.init.constant_(self.mid_gate[-2].bias, -3.0)  # Sigmoid(-3) ≈ 0.05

        # Zero-init so old checkpoints behave identically
        nn.init.zeros_(self.mid_logits_to_h.weight)
        nn.init.zeros_(self.mid_logits_to_h.bias)
    ...
    def forward(self, x, time_cond, class_logits=None):
        ...
        # ---- MID-LEVEL LOGITS FUSION (after middle blocks, before decoder) ----
        if self.logits_mid_cond and (class_logits is not None):
            B, Cb, Hm, Wm = h.shape  # bottleneck spatial size and channels

            # Accept [S, |A|] or [B, S, |A|]
            if class_logits.dim() == 2:                      # [S, |A|]
                class_logits = class_logits.unsqueeze(0)     # [1, S, |A|]
            if class_logits.size(0) == 1 and B > 1:
                class_logits = class_logits.expand(B, -1, -1)  # broadcast to batch

            # Encode per step: |A| -> C_cond
            B2, S, A = class_logits.shape
            flat = class_logits.reshape(B2 * S, A)                   # [B*S, |A|]
            emb  = self.mid_logits_encoder(flat)                     # [B*S, C_cond]
            emb  = emb.view(B2, S, self.logits_proj_channels)        # [B, S, C_cond]

            # Time-align logits sequence to bottleneck width: S -> Wm
            emb = emb.permute(0, 2, 1)                               # [B, C_cond, S]
            if S != Wm:
                emb = torch.nn.functional.interpolate(
                    emb, size=Wm, mode="linear", align_corners=False
                )                                                    # [B, C_cond, Wm]

            # Lift to 2D at bottleneck spatial size by tiling along frequency Hm
            cond_map = emb.unsqueeze(2).expand(B2, self.logits_proj_channels, Hm, Wm)  # [B, C_cond, Hm, Wm]

            # Project to bottleneck channels and add with a learnable gate
