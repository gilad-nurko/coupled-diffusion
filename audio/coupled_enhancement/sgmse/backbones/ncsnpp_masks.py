...
@BackboneRegistry.register("ncsnpp_masks")
class NCSNppMask(nn.Module):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--attn_resolutions", type=int, nargs='+', default=[16]) # was 16
    ...
    def forward(self, x, time_cond):
        ...
        def _match_hw(ref, x):
            """Center pad/crop x so spatial dims match ref. Shapes: (B,C,H,W)."""
            ...
            pad_w = max(Wr - Wx, 0)
            if pad_h or pad_w:
                # F.pad order for 2D: (w_left, w_right, h_top, h_bottom)
        ...
        if h.shape[-2] != H_in or h.shape[-1] != W_in:
            pad_h = max(H_in - h.shape[-2], 0)
            pad_w = max(W_in - h.shape[-1], 0)
            if pad_h or pad_w:
