...
@BackboneRegistry.register("ncsnpp_logits_conditioned")
class NCSNppLogits(nn.Module):
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        # Add logits conditioning modules
        self.class_processor = nn.Sequential(
            nn.Linear(10, 2*nf),
            nn.ReLU(),
            nn.Linear(2*nf, 2*nf)
        )
        self.class_film_generator = nn.Linear(2*nf, 4*nf)
    
    def apply_class_conditioning(self, h, class_emb):
        film_params = self.class_film_generator(class_emb)
    ...
    def forward(self, x, time_cond, class_logits=None):
        ...
        # Apply logits conditioning at bottleneck if provided
        if class_logits is not None:
            if class_logits.dim() == 1:
                class_logits = class_logits.unsqueeze(0)
            class_emb = self.class_processor(class_logits)
            h = self.apply_class_conditioning(h, class_emb)
