...
# # Download dEchorate dataset
# if [ -d "$data_dir/dEchorate" ]; then
#     echo "[Warning] Skip download of dEchorate. The directory $data_dir/dEchorate already exists."
# else
#     mkdir -p "$data_dir/dEchorate/sofa"

#     # These 4 single files are OK with gdown
#     gdown 1pFEI_KEwZROR1EXUHbx7OMLryY6NN47O -O "$data_dir/dEchorate/"
#     gdown 1fsDNVwalMYrI9pq0Q3BI7PFBr0Z_L9PB -O "$data_dir/dEchorate/"
#     gdown 1sErbvkuvSwoBlHXz7ssQM8VhrAD1_4mj -O "$data_dir/dEchorate/"
#     gdown 1zFs4P2pRkX-IcTlvGMoc7lLca9xihJNW -O "$data_dir/dEchorate/"

#     # ---- Use rclone with a SCOPED remote (gdrv_ears) ----
#     if ! command -v rclone >/dev/null 2>&1; then
#         echo "[Error] rclone not found. Install it (or you'll hit gdown's 50-file limit)."
#         echo "  curl -Of https://downloads.rclone.org/rclone-current-linux-amd64.zip && unzip -a rclone-current-linux-amd64.zip"
#         echo "  cp rclone-*-linux-amd64/rclone ~/.local/bin/ && export PATH=\$HOME/.local/bin:\$PATH"
#         exit 1
#     fi

#     # Ensure the scoped remote exists (rooted at the dEchorate folder)
#     if ! rclone listremotes 2>/dev/null | grep -q '^gdrv_ears:'; then
#         echo "[Error] rclone remote 'gdrv_ears' is not configured."
#         echo "Create it once:"
#         echo "  rclone config create gdrv_ears drive scope=drive.readonly root_folder_id=1iKjhy7QvdQ38HwxQ5ZCBI-eCQTuVLlgt"
#         echo "If headless: use 'rclone authorize \"drive\"' on a machine with a browser and paste the token."
#         exit 1
#     fi

#     echo "Using rclone (remote 'gdrv_ears') to download only the dEchorate SOFA folder..."
#     rclone copy gdrv_ears: "$data_dir/dEchorate/sofa" \
#         --include "*.zip" --include "*.sofa" \
#         --max-depth 1 \
#         --checkers=8 --transfers=8 --progress
#     # -----------------------------------------------

#     echo "Extract any zip archives in $data_dir/dEchorate/sofa (if present)..."
#     shopt -s nullglob
#     for file in "$data_dir/dEchorate/sofa/"*.zip; do
#         echo "Extract $file"
#         n_files=$(unzip -l "$file" | tail -n 1 | xargs echo -n | cut -d' ' -f2)
#         unzip -o "$file" -d "$data_dir/dEchorate/sofa/" | tqdm --unit files --unit_scale --total "$n_files" > /dev/null
#         rm -f "$file"
