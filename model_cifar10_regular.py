...
class MNISTDiffusion(nn.Module):
    def __init__(self,image_size,in_channels,time_embedding_dim=256,timesteps=1000,base_dim=32,dim_mults= [1, 2, 4, 8],guiding_noise_level=0.15,is_ddim=True,corruption_type='pixel_noise',blur_sigma=2.0):
        super().__init__()
        ...
        self.model=Unet(timesteps,time_embedding_dim,in_channels,in_channels,base_dim,dim_mults,is_cond=True)
    ...
    def sampling(self, n_samples, clipped_reverse_diffusion=True, device="cuda", x_cond=None, sampling_steps: int = 150):
        ...
        T = self.timesteps          
        S = int(sampling_steps)     

        # pick S indices from T-1..0
        step_indices = torch.linspace(T - 1, 0, steps=S, device=device).long()

        for i in tqdm(step_indices, desc=f"Sampling ({S} steps)"):
            noise = torch.randn_like(x_t)
            t = torch.full((n_samples,), int(i.item()), device=device, dtype=torch.long)
    ...
    @torch.no_grad()
    def _reverse_diffusion(self,x_t,t,noise,cond=None):
        '''
    ...
    @torch.no_grad()
    def _reverse_diffusion_with_clip(self,x_t,t,noise,cond=None): 
        '''
